...
struct Skeleton {
    ...
    // // On récupère l'index de l'articulation finale qui doit se rapprocher de la cible
    // Vec3 currentEndEffector = transfoIK.articulations_transformed_position[targetArticulation];
    
    // // Calculer la distance initiale entre l'articulation finale et la position cible
    // double dist = (currentEndEffector - targetPosition).length();
    
    // unsigned int iter = 0;

    // // Répéter jusqu'à ce que la distance soit inférieure à l'epsilon ou qu'on atteigne l'itération max
    // while (dist > epsilonPrecision && iter < maxIterNumber) {
    //     // Parcourir la chaîne d'articulations, en commençant par celle qui contrôle l'articulation cible
    //     for (int i = bones.size() - 1; i >= 0; --i) {
    //         Bone &bone = bones[i];

    //         // Position actuelle de l'articulation courante
    //         Vec3 articulationPos = transfoIK.articulations_transformed_position[bone.joints[0]];

    //         // Vecteur de l'articulation courante vers la cible
    //         Vec3 toTarget = targetPosition - articulationPos;

    //         // Vecteur de l'articulation courante vers l'articulation finale (end effector)
    //         Vec3 toEndEffector = currentEndEffector - articulationPos;

    //         // Si les vecteurs sont trop courts, on saute cette articulation
    //         if (toTarget.length() < epsilonPrecision || toEndEffector.length() < epsilonPrecision)
    //             continue;

    //         // Calculer la rotation qui aligne le vecteur toEndEffector avec le vecteur toTarget
    //         Mat3 rotation = Mat3::getRotationMatrixAligning(toEndEffector, toTarget);

    //         // Appliquer la rotation à l'articulation courante
    //         transfoIK.bone_transformations[i].localRotation = rotation * transfoIK.bone_transformations[i].localRotation;

    //         // Recalculer les transformations globales du squelette après la rotation
    //         computeGlobalTransformationParameters(transfoIK);

    //         // Mettre à jour la position de l'articulation finale après la rotation
    //         currentEndEffector = transfoIK.articulations_transformed_position[targetArticulation];

    //         // Recalculer la distance entre l'articulation finale et la position cible
    //         dist = (currentEndEffector - targetPosition).length();

    //         // Si on est suffisamment proche de la cible, on peut arrêter
    //         if (dist <= epsilonPrecision) {
    //             return;
    //         }
    //     }
    //     ++iter;
    ...
    int currentArticulation = targetArticulation;
            ...
            while(!articulations.currentArticulation.isRoot()){
                unsigned int boneIdx = currentArticulation.fatherBone;
                Bone &bone = bones[boneIdx];

                unsigned int parentArticulation = bone.joints[0];

                Vec3 currentPos = transfoIK.articulations_transformed_position[currentArticulation];
                Vec3 parentPos = transfoIK.articulations_transformed_position[parentArticulation];

                Vec3 boneDir = currentPos - parentPos;
                Vec3 targetDir = targetPosition - currentPos;

                Mat3 rotation = Mat3::getRotationMatrixAligning(boneDir, targetDir);

                transfoIK.bone_transformations[boneIdx].localRotation = rotation * transfoIK.bone_transformations[boneIdx].localRotation;

                computeGlobalTransformationParameters(transfoIK);

                currentPos = transfoIK.articulations_transformed_position[targetArticulation];

                // if(currentPos - targetPosition.length() < epsilonPrecision){
                //     return;
                // }

                currentArticulation = parentArticulation;
        }
    }
