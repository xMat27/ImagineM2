...
int main(int argc, char *argv[])
{
		...
		switch (droite_gaughe) 
		{
		  ...
		  Droite_disp.set_title("%s","Cliquez ici") ;
				...
				if (Droite_disp.button() && Droite_disp.mouse_y()>=0) 
				{
					...
					printf("Coordonnées du point : %d ", xd[n]);
					printf("%d \n ", yd[n]);
		  ...
		  Droite_disp.set_title("%s","Image droite") ;
				...
				if (Gauche_disp.button() && Gauche_disp.mouse_y()>=0) 
				{
					...
					printf("Coordonnées du point : %d ", xg[n]);
					printf("%d \n ", yg[n]);
					imageG.draw_circle(xg[n],yg[n],3,blue,3.0,1).display(Gauche_disp);
					droite_gaughe = 'd' ; n++ ;
				} break ;
			default : break;
		}		
	...
	while (!Droite_disp.is_closed() && !Gauche_disp.is_closed() && n < 10)
	{
		Droite_disp.set_title("%s", "Cliquez sur un point dans l'image droite");
		Droite_disp.wait();
		if (Droite_disp.button() && Droite_disp.mouse_y() >= 0)
		{
			yd[0] = Droite_disp.mouse_y();
			xd[0] = Droite_disp.mouse_x();
			imageD.draw_circle(xd[0], yd[0], 3, red, 1.0, 1).display(Droite_disp);

			// Calcul de la droite épipolaire dans l'image gauche
			L[0] = (double)(yg[1] - yg[0]);
			L[1] = (double)(xg[0] - xg[1]);
			L[2] = (double)xg[1] * yg[0] - (double)xg[0] * yg[1];

			// Intersection et affichage de la droite épipolaire
			if (Intersection(L, imageG.width(), imageG.height(), x_inter, y_inter))
			{
